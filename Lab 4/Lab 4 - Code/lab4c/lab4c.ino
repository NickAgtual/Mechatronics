...
// Next the constants for the thermistor are defined
float RThermistor = 100000; 
float Resistor = 100000; 
float Bvalue = 3950; 
float RTempRated = 25; 
 
// The temperature setpoint (in Fahrenheit) is defined below: 
float tempset = 85; 
...
if (buttonreading == 1){   //Check if button is pressed 
      flag = abs(flag-1);  //If pressed change the flag to the opposite value 
             delay(500); 
}     //Delay to negate contact bounce 
 
if (flag==1){      //If flag is 1 turn on circuit (the lines below this will  
        //be modified in subsequent circuits  
     
coilPWM =100;
float error = tempset-Farenheit; //compute error 
int pConstant = 10;     //define proportional constant 
coilPWM =error*pConstant;    //define PWM by multiplying error by constant 
  } 
  else 
 { 
